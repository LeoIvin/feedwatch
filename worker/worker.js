/**
 * JobBot webhook worker — answers Telegram commands instantly.
 *
...
const HELP_TEXT = `<b>JobBot commands</b>
...
async function loadJobsBlob(env) {
  ...
  const resp = await fetch(url, { headers: { "User-Agent": "jobbot-worker" } });
...
async function countBoard(ats, slug) {
  const ua = { "User-Agent": "JobBot/1.0 (personal job-alert tool)" };
  let resp, data;
  switch (ats) {
    case "greenhouse":
